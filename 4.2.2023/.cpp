...
void RobotLine::armCatchReady() {
  mrm_servo->write(140, 0); // Lower the arm.
...
void RobotLine::rightWallFallow() {
  armOpen();
  if (rightFront() > 100)
    go(80, 50);
  else
...
void RobotLine::labirint() {
  ...
  if (rightFront() < 215 && leftFront() < 215) {
    go(60, 60);
  }
